...
For publication-grade experiments, add:

- exact dataset specification and split protocol,
- model checkpoint provenance and training seed control,
- full evaluation tables (ACC, F1, AUC) across clean/noisy/missing audio conditions,
- ablations for static fusion vs adaptive gating,
- calibrated decision threshold selection and external validation set testing.

## 11) Immediate Improvement Backlog

- Add automated tests for `/api/analyze` and `/api/report`.
- Add typed API schemas (Pydantic response models) and OpenAPI examples.
- Version and checksum model weights.
- Replace hardcoded API host in frontend with environment configuration.
- Add proper experiment tracking and metrics artifacts into the repository.

## 12) Citation-Style Summary

If you use this project academically, describe it as:

> A FastAPI-based explainable deepfake detection framework using MobileNetV2 visual features, Wav2Vec2 audio embeddings, and adaptive gated multimodal fusion, with frame-level manipulation scoring and dashboard-driven forensic interpretation.
